...
/// ```
    ...
    pub fn get_hourly_forecasts(&self, period: i8) -> Result<Vec<HourlyForecast>> {
        ...
        let url = format!("{}/forecasts/v1/hourly/{}hour/{:?}", self.base_url, period, self.location.unwrap());
        let url = Url::parse_with_params(&url,&[("apikey",self.api_key.clone()),("details","true".to_string()),("metric","true".to_string())])?;
...
/// ```
    ...
    pub fn get_daily_forecasts(&self, period: i8) -> Result<DailyForecastsAnswer> {
        ...
        let url = format!("{}/forecasts/v1/daily/{}day/{:?}", self.base_url, period, self.location.unwrap());
        let url = Url::parse_with_params(&url,&[("apikey",self.api_key.clone()),("details","true".to_string()),("metric","true".to_string())])?;
        match self.client.get(url).send()?.error_for_status()?.json() {
          Ok(x) => Ok(x),
          Err(x) => Err(x.into())
...
/// ```
    ...
    pub fn get_current_conditions(&self) -> Result<Vec<CurrentCondition>> {
        let url = format!("{}/currentconditions/v1/{:?}", self.base_url, self.location.unwrap());
        let url = Url::parse_with_params(&url,&[("apikey",self.api_key.clone()),("details","true".to_string()),("metric","true".to_string())])?;
        match self.client.get(url).send()?.error_for_status()?.json() {
          Ok(x) => Ok(x),
          Err(x) => Err(x.into())
